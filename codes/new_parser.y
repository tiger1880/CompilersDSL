...
string scale = "1";
string center = "new Point(0, 0, false)";
string center_x = "0";
string center_y = "0";
...
expression:   expression '+' expression { is_member=0; $$.eletype = sumTypeCheck($1.eletype, $3.eletype);$$.text = new string; *$$.text = *$1.text + "+" + *$3.text;}
            | expression '-' expression { is_member=0; $$.eletype = diffTypeCheck($1.eletype, $3.eletype);$$.text = new string;
                                            if ($$.eletype == LINE)
            ...
            | expression '^' expression { is_member=0;$$.eletype = mulTypeCheck($1.eletype, $3.eletype);$$.text = new string;*$$.text = "pow(" + *$1.text + "," + *$3.text + ")";}
            | expression PARALLEL expression {is_member=0; $$.eletype = parallelCheck($1.eletype, $3.eletype);$$.text = new string;*$$.text = "isParallel(" + *$1.text + ", " + *$3.text + ")" ; } //need to change this
            | expression PERPENDICULAR expression  {is_member=0;$$.eletype = perpendicularCheck($1.eletype, $3.eletype); $$.text = new string; *$$.text = "isPerpendicular(" + *$1.text + ", " + *$3.text + ")" ;}  //need to change this
            | PARALLEL inside_norm PARALLEL  {is_member=0; $$.eletype = REAL; $$.text = new string;*$$.text = *$2.text;} 
            | '-' expression %prec NEG {is_member=0;if (!arithCompatible($2.eletype)) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = $2.eletype; $$.text = new string;*$$.text = "-" + *$2.text;} 
            | UNARY member_access {is_member=0; /*if(global_space && decl)semanticError("Error: incorrect global declaration");*/ if(!($2.eletype == INT || $2.eletype == REAL)) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = $2.eletype; $$.text = new string;*$$.text = *$1.text + *$2.text;}
            | member_access UNARY {is_member=0;/*if(global_space && decl)semanticError("Error: incorrect global declaration");*/ if(!($1.eletype == INT || $1.eletype == REAL)) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = $1.eletype;  $$.text = new string;*$$.text = *$1.text + *$2.text;}
            | NOT expression {is_member=0;if (!arithCompatible($2.eletype)) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = BOOL;$$.text = new string;*$$.text = "!" + *$2.text;}
            | expression AND expression {is_member=0;if(!(arithCompatible($1.eletype) && arithCompatible($3.eletype))) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = BOOL; $$.text = new string;*$$.text = *$1.text + "&&" + *$3.text; }
            | expression OR expression {is_member=0;if(!(arithCompatible($1.eletype) && arithCompatible($3.eletype))) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = BOOL; $$.text = new string;*$$.text = *$1.text + "||" + *$3.text; }
            | member_access assign       {is_member=0; /*if(global_space && decl)semanticError("Error: incorrect global declaration"); */if (!(($1.eletype == $2.eletype) || coercible($1.eletype, $2.eletype))) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = $1.eletype;  $$.text = new string;*$$.text = assignTranslation(*$2.text,*$1.text);}
            | expression CMP_OP expression {is_member=0;if(!(arithCompatible($1.eletype) && arithCompatible($3.eletype)) && ($1.eletype!=LABEL || $3.eletype != LABEL)) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = BOOL;$$.text = new string;*$$.text = *$1.text + *$2.text + *$3.text;is_member=0;} 
            ...
            | member_access {is_member = 0; /*if(global_space && decl)semanticError("Error: incorrect global declaration");*/ $$.eletype = $1.eletype;$$.text = new string; *$$.text = *$1.text;}
...
assign:  EQUAL expression {$$.eletype = $2.eletype;$$.text = new string; *$$.text = "=" + *$2.text;}
       ...
       | EQUAL construct {$$.eletype = $2.eletype;$$.text = new string; *$$.text = "=" + *$2.text;}
       | ASSIGN_OP  construct {if(!(arithCompatible($2.eletype))) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = $2.eletype;  $$.text = new string;*$$.text = assignOpTranslation(*$1.text) + *$2.text;}  
       | SUM_ASSIGN_OP  construct  {if(!(arithCompatible($2.eletype) || $2.eletype == LABEL || $2.eletype == POINT)) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = $2.eletype;  $$.text = new string;*$$.text = "+=" + *$2.text;}
       | SUB_ASSIGN_OP construct {if(!(arithCompatible($2.eletype) || $2.eletype == POINT)) semanticError("Error: Semantic error incompatible datatype"); $$.eletype = $2.eletype;  $$.text = new string;*$$.text = "-=" + *$2.text;}
       ;
...
string translateLineArr(string linearr){
       ...
       for (int i = 0;i+2 < args.size();i+=2){
              ...
              output = output + "new Line(" + args[i] + ", " + args[i+2] + ", " + convertLineOp(args[i+1]) + "), ";
       }
